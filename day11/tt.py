...
neighbours = np.rollaxis(neighbours + 1, 3)
print(neighbours.shape)


while True:
    prev_seats = np.copy(grid)
    padded_seats = np.pad(grid,1)
    #print(padded_seats.shape,np.ravel_multi_index(neighbours, padded_seats.shape).shape)
